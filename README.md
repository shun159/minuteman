...
tunneling, automatic AFTR/prefix discovery, DHCPv6 Prefix Delegation, and RFC 4389 Neighbor Discovery
Proxy — all without spawning any external client daemons (`dhclient`, `radvd`, `ndppd`, ...) or shelling
out to `ip`/`sysctl`. Every protocol client and every piece of kernel configuration is done in-process,
either in the XDP fast path or in hand-rolled Go using raw sockets and netlink.
...
DS-Lite, AFTR discovery (both DHCPv6 and HB46PP), DHCPv6-PD, NDProxy, and the DNS proxy are all implemented
and have been verified end-to-end against the `test/netns/` rig (see [Testing](#testing) below) — XDP
programs load, pass the kernel verifier, and process live traffic through a real (simulated) softwire; a
live capture on the simulated AFTR's own decap interface during a DNS-proxied query confirmed zero packets
ever cross the softwire for it.
...
| `-lan` | LAN interface as `iface=gatewayIP[,mtu]`; repeatable, at least one required |
...
Three environment variables select which of minuteman's discovery/provisioning paths the rig exercises,
